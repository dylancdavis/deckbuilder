...
describe('turn transition ability ordering', () => {
  it('resolves board turn-end abilities before the turn advances', () => {
    const ender = makeCard('ender', [{ trigger: { on: 'turn-end' }, effects: marker(1) }])
    const starter = makeCard('starter', [{ trigger: { on: 'turn-start' }, effects: marker(2) }])
  ...
  it('resolves multiple board turn-end abilities in board order', () => {
    const first = makeCard('first', [{ trigger: { on: 'turn-end' }, effects: marker(1) }])
    const second = makeCard('second', [{ trigger: { on: 'turn-end' }, effects: marker(2) }])
    const gameState = createTestGameState({
      cards: {
        drawPile: [makeCard('draw-1', []), makeCard('draw-2', [])],
        hand: [],
